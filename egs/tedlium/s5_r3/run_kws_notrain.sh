...
if [ $stage -le 0 ]; then
  ...
  if [ -f data/local/kws_dict_nosp/lexiconp.txt ]; then
    rm data/local/kws_dict_nosp/lexiconp.txt
  fi
...
if [ $stage -le 250 ]; then
  ...
  cur_results_dir=${results_dir}/no_retrain_12_layer_${keyword}
  ...
  for dset in dev test; do
    ...
    python3 local/confusion.py --reference ${dir}/decode_kws_${dset}/scoring/test_filt.txt \
      --hypothesis ${results_dir}/no_retrain_12_layer_${keyword}/${dset}-10-decoded.txt --keyword $keyword \
